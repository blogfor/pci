<?php echo $header; ?>
  <?php 
        ...
        $desc_str="";
             ...
             $desc_str.=$attribute_group['name']."-";                      
                foreach ($attribute_group['attribute'] as $attribute) {
                $desc_str.= $attribute['name'].":".$attribute['text']."&nbsp; ,";                                              
         		}
             }   
        } 
   ?><head>
<meta property="og:title" content="<?php echo $heading_title; ?> at <?php echo $price; ?>" />
        ...
        <div class="recent-projects">
          ...
          <div class="row">
            ...
            <?php if($product_stock<=0){?>
                    <img src="catalog/view/theme/pccindianew/images/iowa-state-bank-sold-ribbon.png"  style="float: none; height: 50px; top: 0; z-index: 9; left: 100px; position: absolute;"/>
                    <?php } ?>
            ...
            <div class="col-lg-7 col-md-7 col-sm-7 product-info">
              ...
              <div class="cart"  style="padding: 15px; background: #fff; border-bottom: 1px solid #ddd;">
                ...
                <div><?php echo $text_qty; ?>
                  ...
                  <input type="button" value="<?php echo $button_cart; ?>" id="button-cart" class="btn" <?php if($product_stock<=0) echo "disabled"; ?> />
			  ...
			  <div class=""  style="padding: 15px; background: #fff; border-bottom: 1px solid #ddd;">
                <div>
                  <input type="text" name="pin_checking" id="pin_checking" size="16" value="" style="margin-top: 7px; height:36px; text-align:center; font-weight: bold;" placeholder="PIN code" />
                  <input type="button" value="Check Availability" id="availability" class="btn" <?php if($product_stock<=0) echo "disabled"; ?> onclick="javascript:checkavailability($('#pin_checking').val());" />
				</span> </div>
              </div>
...
{
    $.ajax({
		url: 'index.php?route=checkout/checkout/pincode&code=' + val,
		dataType: 'json',
		beforeSend: function() {
                    
			$('#pin_checking').after('<span class="wait">&nbsp;<img src="catalog/view/theme/default/image/loading.gif" alt="" /></span>');
		},
		complete: function() {
			$('.wait').remove();
		...
		success: function(json) {
			if (json['status'] == '1') {
				BootstrapDialog.show({
                                title: 'Hurray!!, this item can be deliver in your area',
                                message: 'We are glad that we can deliver this item in your area.',
                                buttons: [{
                                    id: 'btn-ok',   
                                    label: 'OK',
                                    cssClass: 'btn-primary', 
