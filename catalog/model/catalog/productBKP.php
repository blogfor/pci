...
class ModelCatalogProduct extends Model {
	...
	public function getProduct($product_id) {
		...
		if ($query->num_rows) {
                ...
                $gold_rate=$this->db->query("SELECT * FROM " . DB_PREFIX . "setting WHERE `group`='config' AND `key`='".$query->row['gold_carat']."'");
				
		$gold_rate=$gold_rate->row;
                $gold_rate=$gold_rate['value'];
                
                /*
                 * For Category man charges
                 */
                
                $catgeory=$this->db->query("SELECT * FROM " . DB_PREFIX . "product_to_category WHERE product_id = '" . $query->row['product_id']. "'");
                
                $mancharges=$this->db->query("SELECT * FROM " . DB_PREFIX . "category WHERE category_id='".$catgeory->row['category_id']."'");
				
		$mancharges=$mancharges->row;
                
                
                /*
                 * Net price calculation
                 */
                $GOLD_PRICE=$gold_rate*$query->row['gold_weight'];
                ...
                $NET_PRICE=$GOLD_PRICE+$MAN_CHARGE+$query->row['diamond_price'];    
                    
			return array(
                                'MAN_CHARGE'=>number_format($MAN_CHARGE,2),
                                'GOLD_PRICE'=>number_format($GOLD_PRICE,2),
				...
				'viewed'           => $query->row['viewed'],
                                'gold_carat' => $query->row['gold_carat'],
                                'diamond_price' => number_format($query->row['diamond_price'],2),
                                'gold_weight' => $query->row['gold_weight'],
			);
		} else {
			return false;
		}
		...
		if (!empty($data['filter_manufacturer_id'])) {
			$sql .= " AND p.manufacturer_id = '" . (int)$data['filter_manufacturer_id'] . "'";
                ...
                if (!empty($data['price_range'])) {                    
                    $search_arr=explode("-",$data['price_range']);
                    if(isset($search_arr) && count($search_arr)==2 ){
                        $sql .= " AND (p.price>='".$search_arr[0]."' and p.price<='".$search_arr[1]."')";
                        
                        $data['pricerange_low']=$search_arr[0];
                        $data['pricerange_high']=$search_arr[1];
                    }
                    
                    $search_arr_compair=explode(":",$data['price_range']);                    
                    if(isset($search_arr_compair) && count($search_arr_compair)==2 ){
                        if($search_arr_compair[0]=="GT") $operators=">=";
                        if($search_arr_compair[0]=="LT") $operators="<=";
                        $sql .= " AND p.price".$operators."'".$search_arr_compair[1]."'";                        
                    }
